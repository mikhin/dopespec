 ...
 */
export const generatePolicyIndex = (policies: PolicyDef[]): string => {
  ...
  for (const [modelName, actions] of [...index].sort(([a], [b]) =>
    a.localeCompare(b),
  )) {
    lines.push(`  ${modelName}: {`);

    for (const [action, policyNames] of [...actions].sort(([a], [b]) =>
      a.localeCompare(b),
    )) {
