...
export function OrderCancel(ctx: OrderProps): OrderProps {
  if (ctx.status !== "pending") {
    throw new Error(
      `Cannot cancel: expected status 'pending', got '${ctx.status}'`,
    );
  }
...
export function OrderDeliver(ctx: OrderProps): OrderProps {
  if (ctx.status !== "shipped") {
    throw new Error(
      `Cannot deliver: expected status 'shipped', got '${ctx.status}'`,
    );
  }
...
export function OrderPay(ctx: OrderProps): OrderProps {
  if (ctx.status !== "pending") {
    throw new Error(
      `Cannot pay: expected status 'pending', got '${ctx.status}'`,
    );
  }
  if (!(ctx.total > 0)) {
