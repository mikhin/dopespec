...
export function PetRelease(ctx: PetProps): PetProps {
  if (ctx.status !== "reserved") {
    throw new Error(
      `Cannot release: expected status 'reserved', got '${ctx.status}'`,
    );
  }
...
export function PetReserve(ctx: PetProps): PetProps {
  if (ctx.status !== "available") {
    throw new Error(
      `Cannot reserve: expected status 'available', got '${ctx.status}'`,
    );
  }
...
export function PetSell(ctx: PetProps): PetProps {
  if (ctx.status !== "reserved") {
    throw new Error(
      `Cannot sell: expected status 'reserved', got '${ctx.status}'`,
    );
  }
  if (!(ctx.price > 0)) {
