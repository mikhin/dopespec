...
describe("generatePolicyTests", () => {
    ...
    it("generated policy validator + tests compile under strict tsc", () => {
        ...
        try {
            writeFileSync(join(dir, "package.json"), '{"type":"module"}');
            writeFileSync(join(dir, "order.types.ts"), generateTypes(Order));
            writeFileSync(join(dir, "customer.types.ts"), generateTypes(Customer));
            writeFileSync(join(dir, "order.policies.ts"), generatePolicyValidator([petStorePolicy], policyModelLookup));
            writeFileSync(join(dir, "order.policy.test.ts"), generatePolicyTests("order", [petStorePolicy], policyModelLookup));
            const tsFiles = [
                "order.types.ts",
                "customer.types.ts",
                "order.policies.ts",
